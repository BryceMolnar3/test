...
class PhylogeneticTreeBuilder:
    ...
    def calculate_distance_matrix(self, collation_data, manuscripts):
        ...
        # Apply scaling with noise to create a more interesting tree
        if max_val > min_val:
            print("Scaling distance matrix for better visualization")
            for i in range(n):
                for j in range(i+1, n):
                    if avg_distance_matrix[i,j] > 0:
                        normalized = (avg_distance_matrix[i,j] - min_val) / (max_val - min_val)
                        scaled = target_min + normalized * range_size
                        
                        # Add small random noise to break ties
                        noise = np.random.uniform(-0.03, 0.03)
                        scaled = np.clip(scaled + noise, target_min, target_max)
                        
                        avg_distance_matrix[i,j] = avg_distance_matrix[j,i] = scaled
            
            print(f"After scaling - min:{avg_distance_matrix[~np.eye(n, dtype=bool)].min():.4f}, max:{avg_distance_matrix.max():.4f}")
